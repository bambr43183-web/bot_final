...
# ================= НАСТРОЙКИ =================
BOT_TOKEN = os.getenv("8404813322:AAHW1xd6eoo2SduUTAkYJ1dFaEFlXxxgiR0")  # ваш токен через env
ADMIN_CHAT_ID = int(os.getenv("-1002492131233"))  # ID адмін чату
DB_NAME = "users.db"
...
async def get_age(message: Message, state: FSMContext):
    if not message.text.isdigit():
        await message.answer("Вік має бути числом. Спробуйте ще раз:")
        return
...
async def get_birth(message: Message, state: FSMContext):
    if not re.match(r"^\d{2}\.\d{2}\.\d{4}$", message.text):
        await message.answer("Формат дд.мм.рррр. Спробуйте ще раз:")
        return
